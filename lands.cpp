...
string Volcano::getLongDescription() {
    return "You arrive at a volcano filled with lava!";
}
...
string Volcano::visit(Player& p) {
    ...
    // Chance to get burnt
    if (chance < 10 && chance > 5) {
        description = "The volcano erupts and you get burnt.!";
        p.setHealth(p.getHealth() - 2);
    }
}
...
string Glacier::getLongDescription() {
    return "You arrive at a glacier. It's cold down here.";
}

string Glacier::visit(Player& p) {
    string description = "It's a nice day and much Nothing happens.";
...
string Marsh::visit(Player& p) {
    ...
    // Chance to get bitten by a clocodile
    if (chance < 10 && chance > 5) {
        description = "You are bitten by an clocodile!";
        p.setHealth(p.getHealth() - 1);
        return description;
    }
}
...
string Cave::getLongDescription() {
    return "You arrive at a cave and it fells to rest for couple hours.";
}

string Cave::visit(Player& p) {
    string description = "Upon entering, you smells something.";
...
string Jungle::visit(Player& p) {
    ...
    if (chance < 5){
        description = "You are bitten by a spider!";
        p.setHealth(p.getHealth() - 1);
        return description;
    }
}
...
string Village::getLongDescription() {
    return "You arrived in small camping village";
}

string Village::visit(Player& p) {
    string description = "you decide to approche the camp and ask somehelp";
    int chance = rand() % 100;

    // Chance for a toucan to help out
    if (chance > 50) {
        description = "you have a headche and you are looking for a doctor";
        p.setHunger(p.getHunger() + 1);
        return description;
    }
